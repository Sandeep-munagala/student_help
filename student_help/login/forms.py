...
class EngineeringSignUpForm(UserCreationForm):
    ...
    college = forms.ChoiceField(label="", choices=[], widget=forms.Select(attrs={'class': 'form-control select-college', 'id': 'id_college'}))
    other_college = forms.CharField(label="", max_length=100, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Enter College Name', 'style': 'display: none;'}), required=False)
    ...
    def get_college_choices(self):
        ...
        choices = [('','Select College')]
        colleges = College.objects.all()
        for college in colleges:
            choices.append((college.college, college.college))
